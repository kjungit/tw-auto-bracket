...
export function activate(context: vscode.ExtensionContext) {
  ...
  // 워크스페이스 폴더 변경 이벤트 감지
  const workspaceFoldersChangeDisposable =
    vscode.workspace.onDidChangeWorkspaceFolders(() => {
      findAndParseTailwindConfig();
    });

  // 파일 저장 이벤트 감지 (tailwind.config.js 파일이 수정된 경우)
  const fileChangeDisposable = vscode.workspace.onDidSaveTextDocument(
    (document) => {
      if (
        document.fileName.endsWith("tailwind.config.js") ||
        document.fileName.endsWith("tailwind.config.ts")
      ) {
        findAndParseTailwindConfig();
      }
    }
      ...
      if (event.contentChanges.length > 0) {
        ...
        if (
          ...
          if (languages.includes(document.languageId)) {
            ...
            // class 또는 className 속성 내부인지 확인 - 조건 완화
            // 기존: if (/(class|className)=["'][^"']*$/.test(linePrefix)) {
            ...
            if (wordMatch) {
              ...
              const fullPatternMatch = /^[a-zA-Z][-a-zA-Z]*\d+[a-z%]+$/.test(
                word
              );
  ...
  const provider = vscode.languages.registerCompletionItemProvider(
    ["javascript", "typescriptreact", "javascriptreact", "html", "css", "vue"],
    {
      provideCompletionItems(
        ...
        // class 또는 className 체크 조건 완화 - 백틱 내부와 일반 문자열에서도 작동하도록 수정
        // 기존: if (!/(class|className)=["'][^"']*$/.test(linePrefix)) {
        //   console.log("class 속성 안에 없음");
        //   return [];
        // }

        // 문자열 컨텍스트 내부에 있는지만 확인 (백틱, 작은따옴표, 큰따옴표 모두 허용)
        // 또는 아예 조건 검사를 생략하고 모든 컨텍스트에서 작동하도록 함
        ...
        const propNumberUnitMatch =
          /^([a-zA-Z][-a-zA-Z]*?)(\d+)([a-z%]+)$/.exec(word);
        if (propNumberUnitMatch) {
          ...
          const cleanProperty = isNegative
          ...
          const bracketContent = isNegative
            ? `[-${absoluteNumber}${mappedUnit}]`
            : `[${number}${mappedUnit}]`;
          ...
          console.log(
            "제안 항목:",
            completionItems.map((item) => item.label).join(", ")
          );
          return completionItems;
        }

        // --- 속성+숫자+단위 패턴만 처리하도록 변경 ---
        const directPatternMatch = /^([a-zA-Z][-a-zA-Z]*?)(\d+)([a-z%]+)$/.exec(
          word
        );
        if (directPatternMatch) {
          ...
          // 숫자가 음수인지 확인 (음수는 속성에 하이픈이 있고, 숫자가 양수인 경우)
          const isNegative =
            property.endsWith("-") && !numberVal.startsWith("-");
...
function parseSpacingFromConfig(configContent: string): Record<string, string> {
  ...
  try {
    ...
    const extendMatch = configContent.match(
      /extend\s*:\s*{([^}]*)spacing\s*:\s*{([^}]*)}/
    );
...
export function mapProperty(property: string): string {
  ...
  if (
    lowerProperty.startsWith("translate") &&
    !lowerProperty.includes("-") &&
...
// 테스트를 위해 export하는 함수
export function ParseSpacingFromConfig(
